---
name: "c^2"
description: "Claude-squared — shard work across parallel Claude Code agents using the built-in Agent tool with isolated worktrees. Use /c^2 work to dispatch a single worker, /c^2 swarm to fan out multiple tasks, /c^2 status to track progress, /c^2 msg to communicate with workers. Supports multi-repo dispatch. This is the Agent-tool-native approach — for the multiclaude daemon/binary, use /mc instead."
argument-hint: <work|swarm|status|gather|msg|repo> [args]
---

# Claude² (c^2)

Dispatch Claude agent workers in isolated git worktrees to do real work in parallel using the **Agent tool** (Claude Code's built-in subagent system). Each worker gets a full repo copy, commits to its own branch, and creates a PR via `gh`.

> **Note**: This skill uses Claude Code's native Agent tool for dispatch. For the `multiclaude` daemon-based orchestrator, use `/mc` instead.

## Repo Registry

c^2 can dispatch workers to any registered repo, not just the current one.
...
- Registered repos are referenced with `@name` syntax: `/c^2 work @prime "fix the auth bug"`
- If no `@repo` is specified, defaults to the current working directory

## Subcommands

### `c^2 work "<task>"` (aliases: `w`, bare string)
...
5. **Report** the agent ID and task summary. Track it for `/c^2 status`.

**Examples:**
- `/c^2 work "add input validation to the signup form"`
- `/c^2 work @prime "update the prompt template for v2"`
- `/c^2 "fix the flaky timeout in test_auth.py"` (bare string = work)

### `c^2 swarm "<goal>"` (aliases: `s`, `fan`)
...
**Examples:**
- `/c^2 swarm "add unit tests for all service classes"`
- `/c^2 swarm @prime "migrate all API endpoints from v1 to v2 format"`
- `/c^2 fan "update every Dockerfile to use multi-stage builds"`

### `c^2 status` (aliases: `st`, `ls`)
...
- **Always end with `say`**: Run `say -v Zarvox "Swarm complete. N of M workers landed PRs."` (or a fun variation) so the user gets an audio alert

### `c^2 msg "<agent-id>" "<message>"` (aliases: `m`, `tell`)
...
**Examples:**
- `/c^2 msg abc123 "also update the README when you're done"`
- `/c^2 tell abc123 "skip the migration tests, they're flaky"`

### `c^2 repo` (aliases: `repos`, `r`)

Manage the repo registry.

- **`c^2 repo list`** — show all registered repos
- **`c^2 repo add <name> <path>`** — register a repo: `/c^2 repo add prime ~/work/claude-prime`
- **`c^2 repo rm <name>`** — unregister a repo
- **`c^2 repo scan`** — auto-discover git repos in common locations (`~/work/`, `~/projects/`, `~/src/`) and offer to register them
...
- `/c^2 status` reads these files and surfaces messages

### Message Types (worker -> coordinator)
- **BLOCKED**: Worker can't proceed without input. Coordinator should `/c^2 msg` a response or resume the worker.
...
- **Bare string = work.** If the user writes `/c^2 "do something"` without a subcommand, treat as `work`.
...
- **Message directory lifecycle.** Create `~/.claude/mc-messages/<agent-id>/` when launching. Clean up completed worker directories on `/c^2 gather` (or leave them — they're small).
