---
name: eval-document
description: Evaluate loaded documents against a criteria file, producing a structured findings report. Use when the user asks to "evaluate", "review", or "check" documents against criteria.
...
1. **Gather evidence.** Run the eval script to search for relevant clauses per criterion:
   ...
   uv run --with 'chromadb,sentence-transformers' \
       "${CLAUDE_PLUGIN_ROOT}/skills/eval-document/document-eval.py" "$ARGUMENTS"
   ```
