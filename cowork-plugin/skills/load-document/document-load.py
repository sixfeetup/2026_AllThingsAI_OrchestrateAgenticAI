...
Usage:
    uv run document-load.py <path> [--db <db-path>] [--chroma <chroma-dir>]

Accepts a single document (PDF/DOCX/XLSX) or a ZIP archive of documents.
Calls document-parse.py internally via uv run, then loads results into both stores.
"""
...
def main():
    ...
    result = subprocess.run(
        ["uv", "run", str(PARSER_SCRIPT), str(input_path)],
