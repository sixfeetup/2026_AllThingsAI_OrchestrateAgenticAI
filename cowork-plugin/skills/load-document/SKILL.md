---
name: load-document
description: Extract, parse, and load documents from a ZIP archive or single file into SQLite + ChromaDB. Use when the user asks to "load", "ingest", or "import" a document.
argument-hint: "<path-to-archive-or-file>"
---
...
2. Run the loader to parse and populate both data stores:
   ```bash
   uv run --with 'pymupdf,chromadb,sentence-transformers,python-docx,openpyxl' \
       "${CLAUDE_PLUGIN_ROOT}/skills/load-document/document-load.py" "$ARGUMENTS"
   ```
...
3. Report results to the user:
   ...
   - Confirm both SQLite and ChromaDB are populated
