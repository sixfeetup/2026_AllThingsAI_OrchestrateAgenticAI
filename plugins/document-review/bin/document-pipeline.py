...
Usage:
    uv run --with 'pymupdf,python-docx,openpyxl,chromadb,sentence-transformers' \
        document-pipeline.py [archive] [--criteria FILE ...] [--db PATH] [--chroma PATH]
...
DEFAULT_ARCHIVE = DEMO_DIR / "assets" / "1-RFP 20-020 - Original Documents.zip"
DEFAULT_DB = DEMO_DIR / "data" / "documents.db"
DEFAULT_CHROMA = DEMO_DIR / "data" / "chroma"
DEFAULT_CRITERIA = [
    DEMO_DIR / "assets" / "criteria" / "ip-and-ownership.md",
    DEMO_DIR / "assets" / "criteria" / "general-red-flags.md",
]

LOAD_SCRIPT = SCRIPT_DIR / "document-load.py"
EVAL_SCRIPT = SCRIPT_DIR / "document-eval.py"
...
def run_load(archive: Path, db: Path, chroma: Path) -> bool:
    """Run the load script."""
    cmd = [
        sys.executable, str(LOAD_SCRIPT), str(archive),
...
def run_eval(criteria: Path, db: Path, chroma: Path) -> str | None:
    ...
    cmd = [
        sys.executable, str(EVAL_SCRIPT), str(criteria),
        "--db", str(db), "--chroma", str(chroma),
    ]
